...
@router.get("/sessions/{session_id}/download/fixed",
            ...
            description="""Download a ZIP file containing fixed source code files.
            
## Use Case
Download all analyzed files with security fixes applied. Only available for Git-based sessions.
...
- Fixed source code files (preserving original directory structure)
- README.txt with analysis summary

## Notes
- Only available for Git-based sessions
- Session must be in COMPLETED status
- Files without fixes are included unchanged""")
async def download_fixed_files(
    ...
    git_ref = session_insp.attrs.git_ref.value
...
@router.get("/sessions/{session_id}/download/patches",
            ...
            description="""Download a ZIP file containing .patch files for Git.
            
## Use Case
Download unified diff patches that can be applied with `git apply`.
Only available for Git-based sessions.
...
## Response
Returns a ZIP file containing:
- .patch files (one per modified file)
